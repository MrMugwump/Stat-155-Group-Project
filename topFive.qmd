...
currSet <- c("PTS", "TRB", "AST", "STL")  # You can reduce this to 3 if needed
...
```{r}
top5 <- results %>% arrange(desc(adj_r2)) %>% head(5)
top5
```
```{r}
# Model 1: Full 4-way interaction
model1 <- lm(salary ~ PTS * TRB * AST * STL, data = salariesAndTotals2021)

# Model 2: 3-way interaction (PTS, TRB, AST) plus STL as additive term
model2 <- lm(salary ~ PTS * TRB * AST + STL, data = salariesAndTotals2021)

# Model 3: Two 2-way interactions: (TRB * STL) and (PTS * AST)
model3 <- lm(salary ~ TRB * STL + PTS * AST, data = salariesAndTotals2021)

# Model 4: Two 2-way interactions: (PTS * AST) and (TRB * STL)
model4 <- lm(salary ~ PTS * AST + TRB * STL, data = salariesAndTotals2021)

# Model 5: 3-way interaction (PTS, AST, STL) plus TRB as additive term
model5 <- lm(salary ~ PTS * AST * STL + TRB, data = salariesAndTotals2021)
